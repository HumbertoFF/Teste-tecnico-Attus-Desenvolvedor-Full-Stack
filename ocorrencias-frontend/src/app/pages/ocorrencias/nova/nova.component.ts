...
@Component({
      ...
      <mat-stepper #stepper [linear]="true">
        ...
        <mat-step [stepControl]="clienteForm" label="Dados do Cliente" errorMessage="Corrija os erros">
          <form [formGroup]="clienteForm" novalidate>
            ...
            <div class="step-actions">
              ...
              <button mat-flat-button color="primary" matStepperNext (click)="clienteForm.markAllAsTouched()">
        ...
        <mat-step [stepControl]="enderecoForm" label="Endereço" errorMessage="Corrija os erros">
          <form [formGroup]="enderecoForm" novalidate>
              ...
              <div class="section-card">
                ...
                <div class="form-grid-2">
                  <mat-form-field appearance="fill" class="span-2">
                    ...
                    <input matInput formControlName="nmeLogradouro" />
                  ...
                  <mat-form-field appearance="fill">
                    ...
                    <input matInput formControlName="nroCep" maxlength="8" (input)="onlyNumbers($event)" />
                    ...
                    @if (enderecoForm.get('nroCep')?.hasError('pattern') && enderecoForm.get('nroCep')?.touched) {
                      <mat-error>CEP deve ter 8 dígitos</mat-error>
                    }
                    <mat-hint>Apenas 8 dígitos</mat-hint>
                  </mat-form-field>
                  <mat-form-field appearance="fill">
                  ...
                  <!-- select nativo — substitui mat-select para evitar bug CDK Overlay -->
                  <div class="native-select-wrap">
                    <label class="native-select-label">Estado *</label>
                    <select class="native-select"
                            formControlName="nmeEstado"
                            [class.invalid]="enderecoForm.get('nmeEstado')?.invalid && enderecoForm.get('nmeEstado')?.touched">
                      <option value="" disabled>Selecione</option>
                      @for (uf of ufs; track uf) {
                        <option [value]="uf">{{ uf }}</option>
                      }
                    </select>
                    <span class="native-select-error">
            ...
            <div class="step-actions">
              <button mat-stroked-button matStepperPrevious><mat-icon>arrow_back</mat-icon> Voltar</button>
              <button mat-flat-button color="primary" matStepperNext (click)="enderecoForm.markAllAsTouched()">
                Próximo <mat-icon>arrow_forward</mat-icon>
              </button>
            </div>
          </form>
          ...
          <div class="step-body">
            <div class="section-card">
              <div class="stitle-row">
                ...
                <span class="section-title" style="margin:0">Arquivos Anexos <span class="opt-tag">opcional</span></span>
              </div>
              <app-upload-zone (filesChange)="files = $event" hint="PNG, JPG, WEBP, PDF — máx 10MB por arquivo" />
            </div>
            ...
            @if (error()) {
              <div class="form-error"><mat-icon>warning_amber</mat-icon> {{ error() }}</div>
            }
          </div>
          <div class="step-actions">
            <button mat-stroked-button matStepperPrevious><mat-icon>arrow_back</mat-icon> Voltar</button>
  ...
  `,
  styles: [`
    .nova-wrap{max-width:820px}
    .step-body{padding:20px 0 4px}
    .step-actions{display:flex;gap:10px;align-items:center;padding:4px 0 8px;flex-wrap:wrap}
    .stitle-row{display:flex;align-items:center;gap:8px;margin-bottom:16px}
    .stitle-icon{font-size:18px!important;width:18px!important;height:18px!important;color:var(--accent)}
    .opt-tag{font-size:.7rem;color:var(--muted);background:var(--surface2);padding:2px 7px;border-radius:20px;margin-left:8px;font-weight:500;vertical-align:middle}
    .review-card{background:var(--surface2)!important}
    .review-grid{display:grid;grid-template-columns:1fr 1fr;gap:0 24px}
    .rv-group{font-size:.75rem;font-weight:600;color:var(--accent);text-transform:uppercase;letter-spacing:.5px;margin:0 0 8px}
    .rv{display:flex;justify-content:space-between;align-items:baseline;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:.86rem}
    .rv:last-child{border-bottom:none}
    .rv-l{color:var(--muted);font-size:.8rem;white-space:nowrap;flex-shrink:0}
    .form-error{display:flex;align-items:flex-start;gap:10px;background:rgba(239,68,68,.08);border:1px solid var(--danger);color:#b91c1c;padding:12px 16px;border-radius:8px;font-size:.88rem;margin-bottom:12px}
...
export class NovaComponent {
  ...
  submit() {
    ...
    const cv = this.clienteForm.value; const ev = this.enderecoForm.value;
    this.svc.cadastrar({
      cliente:  { nmeCliente:cv.nmeCliente!, dtaNascimento:cv.dtaNascimento!, nroCpf:cv.nroCpf! },
      endereco: { nmeLogradouro:ev.nmeLogradouro!, nmeBairro:ev.nmeBairro!, nroCep:ev.nroCep!, nmeCidade:ev.nmeCidade!, nmeEstado:ev.nmeEstado! }
    }, this.files).subscribe({
      next: res => { this.snack.open(`Ocorrência #${res.codOcorrencia} cadastrada!`,'',{duration:4000,panelClass:'success-snack'}); this.router.navigate(['/ocorrencias', res.codOcorrencia]); },
      error: e  => { this.loading.set(false); this.error.set(e.error?.detail ?? 'Erro ao cadastrar'); }
    });
  }
}
